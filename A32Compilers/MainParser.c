...
atys_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern atys_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern atys_intg line; /* source code line number - defined in scanner.c */
...
extern atys_void startParser(atys_void);
extern atys_intg startScanner(BufferPointer sc_buf);

static atys_void printParserError(atys_string fmt, ...);
static atys_void displayParser(BufferPointer ptrBuffer);
static atys_long getParserFileSize(atys_string fname);
static atys_void callGarbageCollector(atys_void);
...
atys_intg mainParser(atys_intg argc, atys_string* argv) {

	numParserErrors = 0;			/* Initializes the errors */

	FILE* fi;       /* input file handle */
	atys_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
atys_void printParserError(atys_string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(atys_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
atys_long getParserFileSize(atys_string fname) {
	FILE* input;
	atys_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
atys_void displayParser(BufferPointer ptrBuffer) {
...
atys_void callGarbageCollector(atys_void) {
