...
atys_intg line;								/* Current line number of the source code */
extern atys_intg errorNumber;				/* Defined in platy_st.c - run-time error number */

extern atys_intg stateType[NUM_STATES];
extern atys_string keywordTable[KWT_SIZE];
extern PTR_ACCFUN finalStateTable[NUM_STATES];
extern atys_intg transitionTable[NUM_STATES][CHAR_CLASSES];
...
atys_intg startScanner(BufferPointer psc_buf) {
	...
	for (atys_intg i=0; i<NUM_TOKENS;i++)
...
Token tokenizer(atys_void) {
	...
	atys_char c;	/* input symbol */
	atys_intg state = 0;		/* initial state of the FSM */
	atys_intg lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	atys_intg lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	atys_intg lexLength;		/* token length */
	atys_intg i;				/* counter */
	
	atys_char newc;			// new char
	atys_boln check;
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((atys_intg)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
atys_intg nextState(atys_intg state, atys_char c) {
	atys_intg col;
	atys_intg next;
	col = nextClass(c);
...
atys_intg nextClass(atys_char c) {
	atys_intg val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
...
Token funcID(atys_string lexeme) {
	...
	atys_char lastch = lexeme[length - 1];
...
atys_void printToken(Token t) {
	extern atys_string keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
	...
	case SL_T:
		...
		printf("%s\n", readerGetContent(stringLiteralTable, (atys_intg)t.attribute.codeType));
		break;
	case LPR_T:
		printf("LPR_T\n");
		break;
